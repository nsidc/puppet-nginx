...
define nginx::resource::location (
  ...
  if (($www_root == undef) and ($proxy == undef) and ($location_alias == undef) and ($stub_status == undef) and
      ($fastcgi == undef) and ($uwsgi == undef) and ($location_custom_cfg == undef) and ($internal == false)) {
    fail("Cannot create a location reference without a www_root, proxy, location_alias, \
         fastcgi, uwsgi, stub_status, internal, or location_custom_cfg defined")
  }
